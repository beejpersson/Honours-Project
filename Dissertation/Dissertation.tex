...
In the final stage of development an adaptation was made and Vuforia's Multi-Targets were explored instead. Images were taken of the multiple sides of a cuboid object and a target was created using Vuforia Target Manager. Another application was built in Unity and more attempts were made to detect the object, but again there was no success. This led to the decision to try to change the environment in which the attempted detection was being made. The project was moved from a bright area with wide window that provided directional light to a dimmer area with a diffuse overhead light in the hopes that this would enable detection. Both applications were tested and adjustments made but object detection on the HoloLens still didn't occur.
...
\paragraph{Scanning} %add phone screenies
...
\newpage
\section{Evaluation}
%not testing image targets
%not seeking out a 3D printer
%using shiny pomo
\newpage
\section{Conclusion}
